...
#		 - traffic config, control and statistics
#																			   #
# Module:                                                                      #
#    The sample was tested on a FlexFE LM			                           #
#                                                                              #
################################################################################
...
# Append paths to Ixia python APIs
...
traffic_status = ixiangpf.traffic_config  (
	...
	track_by                =       			'source_ip'							,
	name                   =					"IPv4_TRAFFIC"							,
	emulation_src_handle   	=				    dhcp_client_handle					  	,
	emulation_dst_handle   =					inner_ipv4_2_handle						,
	src_dest_mesh          =        			'one_to_one'                 				,
	route_mesh			=						'one_to_one'								,
	rate_percent         =          			rate_start_value       				,
	frame_size           =          			frame_size_start       				,
)
if traffic_status['status'] != IxiaHlt.SUCCESS:
	ixnHLT_errorHandler('traffic_config', traffic_status)

traffic_item = traffic_status['traffic_item']
print('\n\nConfigured traffic item handle:')
print(traffic_item)
...
if traffic_control_status['status'] != IxiaHlt.SUCCESS:
	ixnHLT_errorHandler('traffic_control', traffic_control_status)
...
if traffic_stats['status'] != IxiaHlt.SUCCESS:
	ixnHLT_errorHandler('traffic_stats', traffic_stats)
...
print('\n\nIPv4 traffic L23 Summary stats:\n')
printDict(traffic_stats)
