...
# Creating a device group in topology
	...
	${result} =  Topology Config  topology_handle=${topology_1_handle}  device_group_name="Device Group 1"  device_group_multiplier=2  device_group_enabled=1
	${status} =  Get From Dictionary  ${result}  status
	Run Keyword If  '${status}' != '1'  FAIL  "Error: Status is not SUCCESS"  ELSE  Log  "Status is SUCCESS"
...
# Creating and Adding IPv4-prefix pool under Network Group1
	Log  Creating ipv4 prefix network address
	...
	Log  Creating and Adding IPv4-prefix pool under Network Group1
...
# This will Create PIMv4 Stack on top of IPv4 Stack of Topology1
	Log  Creating PIMv4 Stack on top of IPv4 Stack of Topology2
...
# Creating and Adding IPv4-prefix pool under Network Group2
	...
	Log  Creating and Adding IPv4-prefix pool under Network Group2
...
#Enabling Bootstrap for Topology1
	...
	${result} =  Emulation Pim Config  handle=${pimV4Interface_1_handle}  mode=modify  ip_version=6  bootstrap_enable=1
	${status} =  Get From Dictionary  ${result}  status
	Run Keyword If  '${status}' != '1'  FAIL  "Error: Status is not SUCCESS"  ELSE  Log  "Status is SUCCESS"
