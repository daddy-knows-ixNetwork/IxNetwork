...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how to use NGPF EVPN Type 5 Route      #
#    HLP API.                                                                  #
#                                                                              #
...
#   - Configure EVI(VxLAN) stack over BGP.                                     #
#   - Configure VA behind EVPN VxLAN Device Group.                             #
#   - Add IPv4/IPv6 Subnets behind VAs.                                        #
...
my $network_group_3_status = ixiangpf::network_group_config ({
    ...
    ipv4_prefix_network_address           => "5.5.5.5",
...
my $network_group_60_status = ixiangpf::network_group_config ({
    protocol_handle                 =>  $networkGroup_3_handle,
    protocol_name                   =>  "{Subnet 1 behind VA 1}",
    multiplier                      =>  1,
    enable_device                   =>  1,
    connected_to_handle             =>  $macPools_1_handle,
    type                            =>  'ipv4-prefix',
...
my $ipv4PrefixPools_40_handle = ixiangpf::status_item('ipv4_prefix_pools_handle');
my $networkGroup_60_handle =  ixiangpf::status_item('network_group_handle');
...
my $network_group_7_status = ixiangpf::network_group_config ({
    ...
    mac_pools_prefix_length               => "48",
    mac_pools_use_vlans                   => "0",
    mac_pools_mac                         => "44.44.44.44.44.44",
    ipv4_prefix_network_address           => "6.6.6.6",
    ipv4_prefix_network_address_step      => "0.1.0.0",
    ipv4_prefix_length                    => "32",
});
$HashRef = ixiangpf::get_result_hash();
...
my $network_group_80_status = ixiangpf::network_group_config ({
    protocol_handle              =>    $networkGroup_7_handle,
    protocol_name                =>    "{Subnet 2 behind VA 2}",
    multiplier                   =>    1,
    enable_device                =>    1,
    connected_to_handle          =>    $macPools_2_handle,
    type                         =>    "ipv4-prefix",
    ipv4_prefix_network_address  =>   "205.1.0.0",
    ipv4_prefix_length           =>    24,
    ipv4_prefix_multiplier       =>    1,
});
$HashRef = ixiangpf::get_result_hash();
...
my $ipv4PrefixPools_70_handle = ixiangpf::status_item('ipv4_prefix_pools_handle');
my $networkGroup_80_handle = ixiangpf::status_item('network_group_handle');

my $network_group_12_status = ixiangpf::emulation_bgp_route_config ({
    handle                       =>  $networkGroup_80_handle,
    mode                         =>  "create",
    active                       =>  1,
    max_route_ranges             =>  1,
    label_step                   =>  1,
...
my $disable_ipPrefix = ixiangpf::emulation_bgp_route_config ({
   handle => '/topology:1/deviceGroup:1/networkGroup:1/deviceGroup:1/networkGroup:1/networkGroup:1/ipv4PrefixPools:1/evpnIPv4PrefixRange:1',
   mode => 'disable'});
$HashRef = ixiangpf::get_result_hash();
$command_status = $HashRef->{'status'};
if ($command_status != $ixiangpf::SUCCESS) {
...
print "On The Fly enable BGP EVPN IPv4 Prefix Range\n";
#(handle : user needs to create and provide handle for cMacProperties, as scriptgen does not return this handle by default)
my $enable_ipPrefix = ixiangpf::emulation_bgp_route_config ({
   handle => '/topology:1/deviceGroup:1/networkGroup:1/deviceGroup:1/networkGroup:1/networkGroup:1/ipv4PrefixPools:1/evpnIPv4PrefixRange:1',
   mode => 'enable'});
$HashRef = ixiangpf::get_result_hash();
$command_status = $HashRef->{'status'};
...
$_result_ = ixiangpf::traffic_config ({
    ...
    endpointset_count       =>        '1',
    emulation_src_handle    =>        [[$networkGroup_60_handle]],
    emulation_dst_handle    =>        [[$networkGroup_80_handle]],
    name                    =>        'Inter_subnet_traffic',
