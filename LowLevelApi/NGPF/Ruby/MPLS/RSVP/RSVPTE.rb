...
@ixNet.commit()
puts('Changing Label Value for second RSVPTE router to multiple value')
@ixNet.setMultiAttribute(@ixNet.getAttribute(rsvpte2, '-labelSpaceStart') + '/counter',
        '-direction',  'increment',
        '-start', '7001',
        '-step', '1')
@ixNet.commit()
puts('Changing Label Space end first RSVPTE router to single value')
...
puts('Changing Label Space end second RSVPTE router to single value')
...
puts('add ipv4 loopback1 for RSVP Leaf Ranges')
...
puts('Adding RSVPTE LSPs over "IPv4 Loopback 1"')
rsvpteLsps1 = @ixNet.add(loopback1, 'rsvpteLsps')
@ixNet.commit()

rsvpteLsps1 = @ixNet.remapIds(rsvpteLsps1)[0]
puts('Adding RSVPTE LSPs over "IPv4 Loopback 2"')
...
###############################################################################
puts("Fetching RSVPTE Basic Learned Info")
