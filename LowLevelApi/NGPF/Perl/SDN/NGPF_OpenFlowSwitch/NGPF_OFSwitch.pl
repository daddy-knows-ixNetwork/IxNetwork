...
$ixNet->add($ip1, 'openFlowSwitch');
$ixNet->commit();

my $openFlowSwitch1 = ($ixNet->getList($ip1, 'openFlowSwitch'))[0];
print "of Switch is : $openFlowSwitch1\n";
my $openflowSwchannels = $ixNet->add($openFlowSwitch1, 'OFSwitchChannel');
$ixNet->commit();
print "OFchannel is : $openflowchannels\n";

sleep(2);
my $openflowSwChannellist = ($ixNet->getList($openFlowSwitch1, 'OFSwitchChannel'))[0];
$ixNet->setAttribute($openflowSwchannels, '-auxConnectionsPerChannel', '1');
$ixNet->commit();
sleep(2);

$ixNet->setMultiAttribute($openFlowSwitch1, '-numberOfTableRanges', '1');
$ixNet->commit();
sleep(2);



my $switchTableList = ($ixNet->getList($openFlowSwitch1, 'switchTablesList'))[0];
print "$switchTableList";


my $networkTopologyObj =($ixNet->add($t1dev1, 'networkTopology'));
$ixNet->commit();
my $networkTopologyObjRing = ($ixNet->add($networkTopologyObj, 'netTopologyRing'));
$ixNet->commit();
...
my $viewPage = '::ixNet::OBJ-/statistics/view:"OF Switch Per Port"/page';
my @statcap  = $ixNet->getAttribute($viewPage, '-columnCaptions');
my @rowvals  = $ixNet->getAttribute($viewPage, '-rowValues');
my $index    = 0;
my $statValueList= '';
...
my $EthernetDestVal = ($ixNet->getAttribute($swPortActive, '-etherAddr'));
...
# print learned info                                                          #
###############################################################################
print("Printing Learned info!!!\n");
$ixNet->execute('getOFChannelLearnedInfo', $openFlowSwitch1, '1');
sleep(5);

print("Print OF Channel Learned Info \n");
my $learnedInfoList = ($ixNet->getList($openFlowSwitch1, 'learnedInfo'))[0];
...
$ixNet->execute('getOFSwitchFlowStatLearnedInfo', $openFlowSwitch1, '1');
sleep(5);

print("Print OF Switch Flow Learned Info \n");
my $learnedInfoList = ($ixNet->getList($openFlowSwitch1, 'learnedInfo'))[0];
my $learnedInfo = ($learnedInfoList, 'end')[0];
my $table = ($ixNet->getList($learnedInfo, 'table'))[0];
my @learnedInfoColumnsList = ($ixNet->getAttribute($table, '-columns'));
