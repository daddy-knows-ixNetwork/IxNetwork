...
my $viewPage = '::ixNet::OBJ-/statistics/view:"OF Controller Per Port"/page';
my @statcap  = $ixNet->getAttribute($viewPage, '-columnCaptions');
my @rowvals  = $ixNet->getAttribute($viewPage, '-rowValues');
my $index    = 0;
my $statValueList= '';
...
my $OfChanneLearnedinfoList1 = ($ixNet->getList($openFlowController1, 'learnedInfoUpdate'))[0];
my $OfChanneLearnedinfoList = ($ixNet->getList($OfChanneLearnedinfoList1, 'oFChannelLearnedInfo'))[0];
