...
$ixNet->setAttribute($topo1, '-name', 'Topology 1 for TRILL');
$ixNet->setAttribute($topo2, '-name', 'Topology 2 for TRILL');
...
print("Adding TRILL interfaces\n");
$ixNet->add($mac1, 'isisTrill');
$ixNet->add($mac2, 'isisTrill');
$ixNet->commit();

my $trillIf1 = ($ixNet->getList($mac1, 'isisTrill'))[0];
my $trillIf2 = ($ixNet->getList($mac2, 'isisTrill'))[0];

print("Setting discard LSP off in TRILL Routers\n");
my $trillRouter1 = ($ixNet->getList($t1dev1, 'isisTrillRouter'))[0];
my $trillRouter2 = ($ixNet->getList($t2dev1, 'isisTrillRouter'))[0];
my $mv1 = $ixNet->getAttribute($trillRouter1, '-discardLSPs');
my $mv2 = $ixNet->getAttribute($trillRouter2, '-discardLSPs');
...
print("Setting Mulitcast IPv4 group in TRILL Router 2\n");
$ixNet->setAttribute($trillRouter2, '-trillMCastIpv4GroupCount', 1);
$ixNet->commit();

my $trillMcastIpv4GroupList = ($ixNet->getList($trillRouter2, 'trillMCastIpv4GroupList'))[0];
my $mvMcastAddrCount = $ixNet->getAttribute($trillMcastIpv4GroupList, '-mcastAddrCnt');
my $mvStartMcastAddr = $ixNet->getAttribute($trillMcastIpv4GroupList, '-startMcastAddr');
...
print("Setting Multicast MAC Groups in TRILL Router 2\n");
$ixNet->setAttribute($trillRouter2, '-trillMCastMacGroupCount', 1);
$ixNet->commit();

my $trillMCastMacGroupList = ($ixNet->getList($trillRouter2, 'trillMCastMacGroupList'))[0];

my $mvMcastAddrCount = $ixNet->getAttribute($trillMCastMacGroupList, '-mcastAddrCnt');
my $mvStartMcastAddr = $ixNet->getAttribute($trillMCastMacGroupList, '-startMcastAddr');
...
print("Setting Mulitcast IPv6 group in TRILL Router 2\n");
$ixNet->setAttribute($trillRouter2, '-trillMCastIpv6GroupCount', 1);
$ixNet->commit();

my $trillMcastIpv6GroupList = ($ixNet->getList($trillRouter2, 'trillMCastIpv6GroupList'))[0];
my $mvMcastAddrCount = $ixNet->getAttribute($trillMcastIpv6GroupList, '-mcastAddrCnt');
my $mvStartMcastAddr = $ixNet->getAttribute($trillMcastIpv6GroupList, '-startMcastAddr');
...
print("Adding TRILL Simulated Egde\n");
$ixNet->add($mac3, 'isisTrillSimRouter');
$ixNet->add($mac4, 'isisTrillSimRouter');
$ixNet->commit();

print("Adding MAC Pools behind TRILL Simulated Edge Device\n");
...
my $viewPage = '::ixNet::OBJ-/statistics/view:"TRILL RTR Per Port"/page';
...
###############################################################################
print("Fetching TRILL Learned Info\n");
$ixNet->execute('getLearnedInfo', $trillIf1, 1);
sleep(5);
my $linfo1 = ($ixNet->getList($trillIf1, 'learnedInfo'))[0];
...
my $table5 = $linfoTables[4];
my $table6 = $linfoTables[5];
...
my @values = $ixNet->getAttribute($table5, '-values');
my @column = $ixNet->getAttribute($table5, '-columns');
print("***************************************************\n");
for (my $index = 0; $index < @values; $index++) {
    my $rowValue = $values[$index];
		for (my $col = 0; $col < @column; $col++) {
...
my @values = $ixNet->getAttribute($table6, '-values');
my @column = $ixNet->getAttribute($table6, '-columns');
print("***************************************************\n");
for (my $index = 0; $index < @values; $index++) {
    my $rowValue = $values[$index];
		for (my $col = 0; $col < @column; $col++) {
