...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how to use Statistics API              #
#    It will create Custom view 					                           #
...
print ("\t - Creating the custom view with name -> Custom View - IPv4  ...\n");
my $cvName = "Custom View - IPv4";
my $protocol = "IPv4";
my $grLevel = "Per Device Group";
my $sortExpr  = "[Device Group] = desc";

#print "- creating view $cvName , with protocol $protocol , grouping level $grLevel");
...
$ixNet->commit();
my @viewList = $ixNet->getList($mv, 'view');
$view = '::ixNet::OBJ-/statistics/view:"Custom View - IPv4"';
...
print "\t - setting protocol $protocol for the filter.\n";
...
print "\t - selecting $grLevel for the filter grouping level." ;
...
my @fil  = $ixNet->getList($view, 'layer23NextGenProtocolFilter');
$ixNet->setAttribute(@fil[0], '-advancedCVFilter', $trackingFilter);
$ixNet->commit();
...
foreach my $stats (@statsList){
$ixNet->setAttribute($stats,'-enabled', 'true');
$ixNet->commit();
}
...
################################################################################
# Retrieve the custom statistics.
################################################################################

print("Fetching all the stats for $view\n");
my $viewPage = $view.'/page';
my @statcap  = $ixNet->getAttribute($viewPage, '-columnCaptions');
my @rowvals  = $ixNet->getAttribute($viewPage, '-rowValues');
my $index    = 0;
my $statValueList= '';
