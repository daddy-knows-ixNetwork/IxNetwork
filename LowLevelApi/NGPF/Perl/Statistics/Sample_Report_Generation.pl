...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how to use NGPF Report API             #
...
my @my_resource = @_;
	...
	my $reportName = "MasterTemplate";
	my $templateName    = "None";
	my $outputPath    = "c:\\Regression\\reporter";
	my $outputFormat = "pdf";
	my $saveResults    = "saveDetailedResults";
	print ("- generate report\n");

	my $defaultTemplateDir = "C:\\Program Files (x86)\\Ixia\\IxReporter-IxN\\data\\Templates";
	my $templateFile = $defaultTemplateDir;
	$ixNet->setMultiAttribute('/reporter/generate',
                '-outputFormat', $outputFormat,
                '-outputPath', join($outputPath,$reportName.".".$outputFormat));
    $ixNet->commit();
    $ixNet->execute('saveDetailedResults', '/reporter/saveResults');
    my $timeOut=300;
    my $counter = 1;
	while ($ixNet->getAttribute('/reporter/saveResults', '-state') ne 'done') {
		print " -- saving detailed results, wait 10 more seconds\n";
        sleep(10);
        if ($counter > $timeOut) {
            print ("Report save Result is taking more than specified timeout time\n");
		}
	 $counter++;
	}
    my $timeOut = 300;
    ...
    while ($ixNet->getAttribute('/reporter/generate', '-state') ne "done") {
        sleep(10);
        if ($counter > $timeOut) {
            exit;
			}
    ...
    my $repGenState = $ixNet->getAttribute('/reporter/generate', '-state');
    if ($repGenState eq "inProgress") {
        print ("Detailed report generation did not complete within $timeOut seconds\n");
		return 1;
		} elsif ($repGenState eq"done") {
        print "- report generation complete\n";
		} elsif ($repGenState eq "failed") {
