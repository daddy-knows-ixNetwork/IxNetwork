...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how to use NGPF Quick Flow Group  API  #
...
my $quick_flow_group = $ixNet->add('/traffic', 'trafficItem');
...
my $endpoint_set = $ixNet->add($quick_flow_group, 'endpointSet');
$ixNet->setMultiAttribute($endpoint_set,
		'-destinations', $vportTx.'/protocols',
		'-sources', $vportRx.'/protocols');
$ixNet->commit();
...
my @highlevelstream1 = $ixNet->getList($quick_flow_group, 'highLevelStream');
$ixNet->setAttribute(@highlevelstream1[0].'/frameSize', '-fixedSize', '120');
$ixNet->setAttribute(@highlevelstream1[0].'/frameRate',	'-rate', '500');
$ixNet->setAttribute(@highlevelstream1[0].'/frameRate',	'-type', 'framesPerSecond');
$ixNet->commit();
...
my @stackList = $ixNet->getList(@highlevelstream1[0], 'stack');
foreach my  $stack (@stackList){
if (index($stack, "ethernet-") != -1) {
	my @fieldList = $ixNet->getList($stack, 'field');
	foreach my $field (@fieldList){
		if (index($field, "ethernet.header.sourceAddress-") != -1) {
			$ixNet->setAttribute($field, '-singleValue', '11:00:00:00:00:00');
			print "Setting the Source Mac:\n"
			}
		}
		}
    $ixNet->commit();
