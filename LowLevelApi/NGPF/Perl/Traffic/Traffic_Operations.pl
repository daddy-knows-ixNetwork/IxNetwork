...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how to use NGPF Traffic Operation API  #
...
sub createBasicIPv4TrafficItem {
my @my_resource = @_;
my $ixNet    = $my_resource[0];
my $name = $my_resource[1];
my $topo1 = $my_resource[2];
my $topo2 = $my_resource[3];
print ("- creating traffic item: %s \n",$name);
...
$ixNet->setMultiAttribute($trafficItem1, '-name', $name ,
...
sub setIngressTrackingForTI {
my @my_resource = @_;
my $ixNet    = $my_resource[0];
my $ti = $my_resource[1];
my $trackingList = $my_resource[2];

    my $tiName = $ixNet->getAttribute($ti, '-name');
    print "--- Traffic Item: $tiName setting ingress tracking $trackingList \n";
...
sub setFirstEgressTrackingForTI{
	my @my_resource = @_;
	my $ixNet    = $my_resource[0];
	my $ti = $my_resource[1];
	my $stack = $my_resource[2];
	my $field = $my_resource[3];
    my $tiName = $ixNet->getAttribute($ti, '-name');
    print "--- Traffic Item: $tiName setting eggress tracking to field $field for stack $stack\n";
    $ixNet->setAttribute($ti, '-egressEnabled', 'True');
    my @et = $ixNet->getList($ti, 'egressTracking');
    $ixNet->setAttribute(@et[0], '-encapsulation', 'Any: Use Custom Settings');
    $ixNet->setAttribute(@et[0], '-offset', 'CustomByField');
    $ixNet->commit();
	my @stackList = $ixNet->getList(@et[0].'/fieldOffset', 'stack');
    foreach my $mstack (@stackList) {
		if (index($mstack, $stack) != -1) {
            my @fieldList = $ixNet->getList($mstack, 'field');
            foreach my $mfield (@fieldList) {
				if (index($mfield, $field) != -1) {
                    $ixNet->setAttribute($mfield, '-activeFieldChoice', 'True');
                    $ixNet->setAttribute($mfield, '-trackingEnabled', 'True');
                    $ixNet->setAttribute($mfield, '-valueType', 'singleValue');
                    $ixNet->commit();
                }
				}
				}
...
sub setLatencyBinsTrackingForTI {
my @my_resource = @_;
	my $ixNet    = $my_resource[0];
	my $ti = $my_resource[1];
	my $binNo = $my_resource[2];
    my $tiName = $ixNet->getAttribute($ti, '-name');
    print "--- Traffic Item: $tiName setting latency bins tracking $binNo\n";
    my @latencyBin = $ixNet->getList($ti.'/tracking', 'latencyBin');
    $ixNet->setAttribute(@latencyBin[0], '-enabled', 'True');
    $ixNet->setAttribute(@latencyBin[0], '-numberOfBins', $binNo);
    $ixNet->commit();
}
...
my $index    = 0;
my $statValueList= '';
foreach my $statValueList (@rowvals) {
    print("***************************************************\n");
    my $statVal = '';
    foreach my $statVal (@$statValueList) {
	    my $statIndiv = '';
		$index = 0;
...
createBasicIPv4TrafficItem($ixNet, "TI 1 IPv4",$topo1,$topo2);
createBasicIPv4TrafficItem($ixNet, "TI 2 IPv4",$topo2,$topo1);
...
my @ti = $ixNet->getList('/traffic','trafficItem');
my $ti1 = @ti[0];
my $ti2 = @ti[1];
...
my @ti_remove = $ixNet->getList('/traffic', 'trafficItem');
$ixNet->remove(@ti_remove[2]);
$ixNet->remove(@ti_remove[3]);
$ixNet->remove(@ti_remove[4]);
$ixNet->commit();
print "- Adding Ingress Tracking for bot Traffic Items\n";
my $trackingList = "sourceDestValuePair0";
