...
#                                                                              #
#    1. It will create 2 BFDv4 topologies, which will be emulated and stats    #
#    	and learned Info fetched                                               #
#                                                                              #
################################################################################
...
my $bfdv41 = ($ixNet->getList($ip1, 'bfdv4Interface'))[0];
my $bfdv42 = ($ixNet->getList($ip2, 'bfdv4Interface'))[0];

my $txInterval1 = $ixNet->getAttribute($bfdv41, '-txInterval');
my $txInterval2 = $ixNet->getAttribute($bfdv42, '-txInterval');
my $minRxInterval1 = $ixNet->getAttribute($bfdv41, '-minRxInterval');
my $minRxInterval2 = $ixNet->getAttribute($bfdv42, '-minRxInterval');
...
############################################################################
print("Adding and Configuring OSPFv2\n");
$ixNet->add($ip1, 'ospfv2');
$ixNet->add($ip2, 'ospfv2');
$ixNet->commit();

my $ospfv21 = ($ixNet->getList($ip1, 'ospfv2'))[0];
my $ospfv22 = ($ixNet->getList($ip2, 'ospfv2'))[0];

my $enableBFD1 = $ixNet->getAttribute($ospfv21, '-enableBfdRegistration');
my $enableBFD2 = $ixNet->getAttribute($ospfv22, '-enableBfdRegistration');
my $networkType1 = $ixNet->getAttribute($ospfv21, '-networkType');
my $networkType2 = $ixNet->getAttribute($ospfv22, '-networkType');
...
my $activation = $ixNet->getAttribute($bfdv41, '-active');
...
sleep(5);
my $linfo  = ($ixNet->getList($bfdv41, 'learnedInfo'))[0];
