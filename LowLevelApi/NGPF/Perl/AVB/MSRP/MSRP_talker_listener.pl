...
my $mac3 = ($ixNet->getList($t2dev2, 'ethernet'))[0];
...
my $talker1 = $ixNet->add($mac1, 'msrpTalker');

$ixNet->commit();

my $talker1 = ($ixNet->remapIds($talker1))[0];
...
my $ptp1 = $ixNet->add($mac1, 'ptp');

$ixNet->commit();

my $ptp1 = ($ixNet->remapIds($ptp1))[0];
...
my $listener1 = $ixNet->add($mac2, 'msrpListener');

$ixNet->commit();

my $listener1 = ($ixNet->remapIds($listener1))[0];



print("Adding gptp clock in topology 2\n");

my $ptp2 = $ixNet->add($mac3, 'ptp');

$ixNet->commit();

my $ptp2 = ($ixNet->remapIds($ptp2))[0];
...
my @linfo = $ixNet->getList($talker1, 'learnedInfo');
...
my @values = $ixNet->getAttribute($streamDb, '-values');

my @column = $ixNet->getAttribute($streamDb, '-columns');
...
my @values = $ixNet->getAttribute($domainDb, '-values');

my @column = $ixNet->getAttribute($domainDb, '-columns');
...
my @values = $ixNet->getAttribute($vlanDb, '-values');

my @column = $ixNet->getAttribute($vlanDb, '-columns');
...
my @linfo = $ixNet->getList($listener1, 'learnedInfo');
...
my $streams = ($ixNet->getList($mac1, 'streams'))[0];

my $multiValue = $ixNet->getAttribute($streams, '-active');
...
my $mcastDestination = [['false', 'none', '22:22:22:22:22:22:00:01',

    '00:00:00:00:00:00:00:00', '1'], ['false', 'none', '22:22:22:22:22:22:00:02',

    '00:00:00:00:00:00:00:00', '1']];

my $mcastReceiver = [[$listener1.'/subscribedStreams', '0', '0', '0'],
