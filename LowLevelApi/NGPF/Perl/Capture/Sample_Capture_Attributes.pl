...
#                                                                              #
#    Revision Log:                                                             #
#    02/12/2015 - Indranil Acharya - created sample                            #
#                                                                              #
################################################################################
...
#    It will create IP and traffic over it. This configures the Capture        #
#    settings and start the capture.Start the protocols and traffic.           #
...
################################################################################
# Capture : Configure slice size and buffer for Captures
################################################################################

print "Set slice size for Control capture\n";
$ixNet->setAttribute(@captureObj1[0], '-controlSliceSize', 64);

print "Set slice size for Data capture";
$ixNet->setAttribute(@captureObj2[0], '-sliceSize', 128);
$ixNet->commit();

print "Setting Control Buffer\n";
$ixNet->setAttribute(@captureObj1[0], '-controlBufferBehaviour', 'bufferAfterStopCircular');
$ixNet->setAttribute(@captureObj1[0], '-controlBufferBehaviour', 'bufferLiveNonCircular');
$ixNet->setAttribute(@captureObj1[0], '-controlBufferSize', 33);
$ixNet->commit();

print "Setting Data Buffer\n";
$ixNet->setAttribute(@captureObj2[0], '-captureMode', 'captureTriggerMode');
$ixNet->commit();
...
my @filterPallette = $ixNet->getList(@captureObj2[0], 'filterPallette');
$ixNet->setMultiAttribute(@filterPallette[0],
...
$ixNet->setAttribute(@captureObj2[0].'/trigger', '-captureTriggerExpressionString', 'SA1');

print "Configure Data - Filter\n";
$ixNet->setAttribute(@captureObj2[0].'/filter', '-captureFilterExpressionString', 'SA1');

print "Configure Control - Start Trigger\n";
$ixNet->setAttribute(@captureObj1[0], '-controlCaptureFilter', 'arp and greater 1 and less 1024 and not (ether src 00:11:01:00:00:0a and ether dst ff:ff:ff:ff:ff:ff)');
$ixNet->commit();

print "Configure Control - Filter\n";
$ixNet->setAttribute(@captureObj1[0], '-controlCaptureTrigger', 'arp and greater 1 and less 1024 and not (ether src 00:11:01:00:00:0a and ether dst ff:ff:ff:ff:ff:ff)');
$ixNet->commit();
