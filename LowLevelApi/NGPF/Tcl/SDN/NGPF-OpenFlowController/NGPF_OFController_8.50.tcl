...
namespace eval ::ixia {
    ...
    set ixTclPort   9862
    set ports       {{10.39.64.118 7 1} { 10.39.64.118 7 2}}
}
...
set ofchannel [ixNet add $openFlowController1 "openFlowChannel"]
ixNet setMultiAttribute $ofchannel -name "Controller\ OF\ Channel\ 1"
ixNet commit

set channel_remote [ixNet getAttribute $ofchannel -remoteIp]
set channel_remote_ip [ixNet add $channel_remote "singleValue"]
ixNet setMultiAttribute $channel_remote_ip \
	-value 1.1.1.1
ixNet commit
...
set viewPage {::ixNet::OBJ-/statistics/view:"OF Controller Per Port"/page}
set statcap [ixNet getAttr $viewPage -columnCaptions]
foreach statValList [ixNet getAttr $viewPage -rowValues] {
    foreach statVal $statValList  {
        puts "***************************************************"
...
#If number of controller is more than 1
#ixNet exec getOFChannelLearnedInfo $openFlowController1 {1 2 3}
...
###############################################################################
# Getting OF channel Learned info                                             #
###############################################################################
set learnedInfo [lindex $learnedInfoList 0]
...
###############################################################################
# Getting Auxiliary Learned info                                              #
###############################################################################
set learnedInfo [lindex $learnedInfoList 1]
set table [lindex [ixNet getList $learnedInfo table] 0]
set learnedInfoColumnsList [ixNet getAttr $table -columns]
set learnedInfoValuesList [ixNet getAttr $table -values]
set row2 [lindex $learnedInfoValuesList 0]
...
# FEA1462667: 8.50 API call changed for onDemand trigger
set OfChanneLearnedinfoList [ixNet getL [lindex [ixNet getL $openFlowController1 learnedInfoUpdate] 0] \
...
#If number of channel is more than 1
#ixNet exec sendOnDemandMessage $OfChanneLearnedinfoList {1 2 3 4 5 6 7 8 9}
