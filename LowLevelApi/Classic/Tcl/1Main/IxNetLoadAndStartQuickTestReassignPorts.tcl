...
#
#    - All stats will be copied from the Windows C: drive to the local
#      Linux path where this script is executed.
#
# Note:
#      For back-to-back port experimenting:
...
set ixNetworkVersion 7.40

# true or false to reassign ports.
set reassignPorts true
# This variable of ports will be ignored if $reassignPorts is set to false
set remapPorts {{1 5} {1 6}} ;# card port
...
# Uncomment this out if you want to reassign ports and
# not want to use the saved config ports.
if {$reassignPorts == "true"} {
    puts "\nResassigning ports\n"
    # Add the chassis here
    set sg_chassis [ixNet add $root/availableHardware chassis]
    # ::ixNet::OBJ-/availableHardware/chassis:L13
    if {[regexp "/availableHardware/chassis:L\[0-9]+" $sg_chassis] == 0} {
	puts "Error: $errMsg"
	exit
    }

    SendIxNetCmd "ixNet setAttribute $sg_chassis -hostname $ixiaChassisIp"
    puts "Please wait 40 seconds for all ports to reboot ..."
    SendIxNetCmd "ixNet commit"

    # Physical port list
    set portList {}
    # set portList [list [list $ixiaChassisIp $card $port1] [list $ixiaChassisIp $card $port2]]
    foreach port $remapPorts {
	lappend portList [list $ixiaChassisIp [lindex $port 0] [lindex $port 1]]
    }

    # Virtual port list from the config file
    set vPortList [ixNet getList $root vport]

    # This will assign the virtual ports to the actual ports
    #  ::ixNet::OBJ-/vport:1 ::ixNet::OBJ-/vport:2
    catch {ixTclNet::AssignPorts $portList {} $vPortList true} errMsg
    puts "Info: Assigned ports: $errMsg"
}
...
# All statistic logs are copied from the C: drive to the local Linux path
