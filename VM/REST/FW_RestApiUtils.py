...
# Description:                                                                 #
#    This sample script                                                        #
#    - Contains function definitions for IxNetwork REST operations             #
#                                                                              #
################################################################################
    ...
    if state == "SUCCESS" and url == "":
        ...
        if state != "COMPLETED":
            ...
            while state == "IN_PROGRESS":
                ...
                it = it + 1
                if state == "EXCEPTION":
                    message = ixNetGetAtt(sessionUrl + "/operations/" + url, "message", False)
...
def ixNetLoadConfig(ixNetSessionUrl, configName):
    ...
    # there's no real need to append the content-length to the headers manually, it will be completed automatically
    urlHeadersData = {'content-type': 'application/octet-stream'}#,'content-length': len(configContent)}

    # get the file name (need to match both Windows and Linux path formats)
    if '\\' in configName:
    ...
    try:
        h = httplib2.Http()
        response, content = h.request(uploadUrl, 'POST', configContent, urlHeadersData)
    except Exception as e:
        raise Exception('Got an error code: ', e)

    if configName not in content:
    ...
    try:
        ...
        response, content = h.request(loadConfigUrl, 'POST', json.dumps(dataJson), urlHeadersJson)
        if response.status not in okStates:
            print ('Error while loading config: %s' % response)
            return False
        print ("Waiting for load config to finish...")
...
class RestProtocols:
    ...
    def startProtocols_restartDown(sessionUrl, expectedStats , wait=600 ,waitRestartDown=90):
        ...
        while (RestStatistics.verifyAllSessionsAre(sessionUrl ,expectedStats) and sec < wait) :
                time.sleep(10)
                sec = sec + 10
                if ( sec >= waitRestartDown):
                                topos = ixNetGetList(sessionUrl,'topology')
                                print (topos)
                                for topo in topos :
                                    print (topo['links'][-1]['href'])
                                    topoUrl = sessionUrl.split('/api/v1/')[0] + topo['links'][-1]['href']
                                    print ("topoUrl = " + str(topoUrl))
                                    args = {'args1':[topoUrl]}
...
#######################################################
# Statistics specific stuff
#####################################################
class RestStatistics:

    @staticmethod
    def getViewPageSnapshot(sessionUrl, view, timeout = 10):
        viewList = ixNetGetList(sessionUrl+'/statistics', 'view')
        stats = ""
        for myView in viewList :
            if myView['caption'] == view :
                stats = myView['links'][-1]['href']
        if stats == "":
            raise TestFailedError("View " + view + " was not found !!!")

        stats = stats +'/page'
        statsUrl = sessionUrl.split('/api/v1')[0] + stats

        cols = ixNetGetAtt(statsUrl, 'columnCaptions')

        retList = []
        i = 1
        total = int(ixNetGetAtt(statsUrl, 'totalPages'))
        while i <= total:
            args = {'currentPage':i}
            ixNetSetAtt(statsUrl, args)
            i+=1
            rows = ixNetGetAtt(statsUrl, 'rowValues')
            for row in rows['arg1']:
                a = []
                for stat,val in zip(cols[1:], row[1:]):
                    a.append((stat, val))
    ...
    def verifyAllSessionsAre (sessionUrl , expectedStats):
        ...
        stat = RestStatistics.getViewPageSnapshot(sessionUrl,view)
        print ("sats = " + str(stat))
        if (RestStatistics.checkStatsSilent(stat, expectedStats)) :
            return 1
        else:
            return 0
    ...
    @staticmethod
    def checkStatsSilent(statList, expList, strings = False):
